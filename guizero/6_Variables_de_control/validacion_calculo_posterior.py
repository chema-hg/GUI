...
SE CALCULA AL PULSAR EL BOTON CALCULAR
'''
...
def calculo():
    ...
    resultado.value=round(total, 5) # redondeamos la parte decimal a 5 posiciones.
   

raiz=App(title="Autobus SL       ", width=220)
...
viaje=Spinbox(box.tk, from_=1, to=20, wrap=False, textvariable=num_via, state='readonly')
viaje.grid(column=0, row=1, padx=5, pady=5)

idavu=CheckBox(box, text='Ida y vuelta', grid=[0,2], align='left')

Text(box, text="Clase:", grid=[0,3], align='left')
clase=ButtonGroup(box, options=["Turista", "Primera", "Lujo"], selected="Turista", grid=[0,4], align='left')


Text(box, text="Distancia en Km.", grid=[0,5], align="left")
dist=TextBox(box, grid=[0,6], width=20)
dist.value=1

Text(box, text="Precio:", grid=[0,7], align='left')
coste=TextBox(box, grid=[0,8], width=20)
coste.value=0.10
...
boton1=PushButton(box4, text="Salir", align='right', padx=20, pady=5, command=quit)
